...
function startRedrawCSS() {
    document.querySelectorAll('.redrawable-controls').forEach(e => e.classList.add("redraw"));
}

function finishRedrawCSS() {
    document.querySelectorAll('.redrawable-controls').forEach(e => e.classList.remove("redraw"));
    document.dispatchEvent(new CustomEvent("reinit-minimap"));
}
